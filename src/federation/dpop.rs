...
impl DPopNonceStore {
    ...
    ///
    /// This is distinct from the §8 server-issued-nonce flow above.
    /// JTIs are generated by the client inside each DPoP proof; the
    /// server tracks them in a seen-set and rejects duplicates per
    /// RFC 9449 §11.1. The retention window is the proof's `exp`
    /// claim — once a proof is past expiry, its JTI doesn't need to
    /// be retained because the proof itself would fail signature
    /// validation anyway.
    ///
    /// Returns `Ok(true)` on first sighting (recorded), `Ok(false)`
    /// if the JTI was already recorded (replay) or has already
    /// expired by the wall clock.
    ///
    /// The store is shared with `generate_nonce` /
    /// `check_and_consume_nonce`; the keyspace is shared but the
    /// semantics don't conflict — server nonces are UUIDv4, client
    /// JTIs are unconstrained but in practice also high-entropy.
    pub async fn check_and_record_jti(&self, jti: &str, exp: i64) -> PdsResult<bool> {
        let now = Utc::now().timestamp();
        if exp <= now {
            return Ok(false);
        }
...
impl DPopVerifier {
    ...
    pub async fn verify_dpop_proof(
        ...
        // Distinct from the §8 server-issued nonce challenge flow,
        // which is a v0.3 candidate; for v0.2 we accept any JTI and
        // refuse repeats within the proof's lifetime.
        if !self
            .nonce_store
            .check_and_record_jti(&claims.jti, claims.exp)
            .await?
        {
...
mod tests {
    ...
    async fn test_check_and_record_jti_first_sighting_recorded() {
        ...
        let recorded = store.check_and_record_jti("client-jti-1", exp).await.unwrap();
    ...
    async fn test_check_and_record_jti_replay_rejected() {
        let store = DPopNonceStore::new();
        let exp = Utc::now().timestamp() + 60;
        store.check_and_record_jti("dupe", exp).await.unwrap();
        let replayed = store.check_and_record_jti("dupe", exp).await.unwrap();
    ...
    async fn test_check_and_record_jti_already_expired_rejected() {
        ...
        let recorded = store
            .check_and_record_jti("late", already_expired)
            .await
            .unwrap();
