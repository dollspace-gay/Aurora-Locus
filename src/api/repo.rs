...
async fn create_record(
    ...
    let repo_mgr = RepositoryManager::new(session.did.clone(), (*ctx.actor_store).clone());

    // Create the record
    let (uri, cid, _rev) = repo_mgr
        .create_record(&req.collection, req.rkey.as_deref(), req.record, dummy_signer)
        .await?;
...
async fn put_record(
    ...
    let (cid, _rev) = repo_mgr
        .update_record(&req.collection, &req.rkey, req.record, dummy_signer)
        .await?;
...
async fn apply_writes(
    ...
    // Apply writes
    let (commit_cid, rev) = repo_mgr.apply_writes(req.writes, dummy_signer).await?;
