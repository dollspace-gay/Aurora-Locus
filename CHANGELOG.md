...
### Changed
- `searchAccounts` honors the `q` search term (Phase B #315) — the admin Accounts search box sends `?q=<term>` but the substrate returned the full account list regardless (the `SearchAccountsQuery` struct had no `q` field, so the param was silently dropped). `q` now filters as a case-insensitive substring across handle, DID, and email; email-exact / cursor / limit and the no-`q` "return all" path are unchanged.
- theme switch repaints colors live (Phase B #306) — switching the deployment-default theme reflowed typography immediately but left colors stale until reload, because the operator-following-default stylesheet (`/theme/active.css` with no `?id`) kept the same URL and was served from cache. The `'default'` theme href now carries a `?v=<resolved-theme>` cache-bust (no `?id`, so the server still resolves the deployment-default) so a changed default forces a refetch.
- `prefers-reduced-motion` honoring strengthened + pinned (Phase B #307) — the framework already collapsed animations/transitions under the OS reduced-motion preference (tokens.css universal rule); this adds `scroll-behavior: auto` (smooth-scroll wasn't suppressed) and locks the rule with a regression test. No operator toggle ships — the browser-native media query is the honoring.
- cosmetic-setting opt-out for audited saves (Phase B #308) — `AuroraAuditedSave` gains `cosmetic: true`: a light confirm with no required typed rationale, writing the setting under an auto-filled rationale so the change still lands an audit-chain entry. The theme deployment-default save uses it (a required rationale was friction for a cosmetic setting); destructive ops keep the full typed-confirm + rationale gate.
- operator destructive actions now land in the tamper-evident audit chain (Phase B #303) — repo rebuild, bulk repair, and manual Laquna rotation emit to `audit_chain_entry` (read by `getAuditTrail` / the Audit page) via `insert_chain_entry_pool`, not only the `moderation_event` feed, so the operator's decision + typed-confirm rationale are recorded in the chain. The manual-rotation rationale (previously captured by the modal then discarded) is now threaded through `triggerRotation`. The chain is the operator-decision log: user-initiated block cascades and system cadence-organic rotations stay in the event feed.
- `tools.aurora.admin.getReport` endpoint (Phase B #302) — registered the previously-unrouted single-report fetch (the store method existed; the report-detail page 404'd on the legacy `com.atproto.admin.getReport` NSID). Moderator+, camelCase output; the admin-client wrapper is migrated to it.
- `getSubjectContext` / `getSubjectHistory` account-detail drawers send `did` instead of `subjectDid` (Phase B #301) — both matched the backend's `did` query field, fixing the 400 "missing field `did`" on the subject-context and subject-history drawers.
- runtime-setting `InvalidEvent` errors surface the substrate's message (Phase B #300) — the error-translation layer no longer maps the overloaded `InvalidEvent` code to a moderation-takedown message; it falls through to the substrate's specific text (e.g. "unknown runtime setting key …").
- Config-General + Laquna account-cadence-range fields shown read-only (Phase B #299) — those runtime-setting keys are not registry-backed and nothing consumes them, so the unbacked save paths are disabled (read-only display + a deferral note) rather than presenting saves that always fail; wiring them end-to-end is tracked as follow-up (#305).
- Laquna rotation-history cadence-organic empty-state reworded (Phase B #304) — #238's write-side is wired; the track is empty only until a cadence rotation fires, so the copy no longer cites a backend follow-up.
- operator role on `getSession` (Phase B #297) — `com.atproto.server.getSession` now returns the caller's operator `role` (`moderator`/`admin`/`superadmin`), looked up live from `admin_roles` and omitted for non-operators (standard session shape preserved). The admin UI resolves its sidebar/route gating from this live value instead of a login-cached one, so an operator's tier — and any grant change — is reflected on the next request without re-login (§8.1.6). Fixes the admin UI treating admin/superadmin operators as `moderator`.
- `getModerationMetrics` GET + array query params (Phase B #298) — the `tools.aurora.admin.getModerationMetrics` admin-client wrapper now uses GET (matching its route registration; it was POSTing → 405 on the dashboard metrics panel), and the admin XRPC client expands array query params into repeated keys (`metrics=a&metrics=b`) as the handler's `axum_extra` Query parser requires.
- shared list-page helpers (Arc I) — the four moderation list pages (Appeals, Audit, Events, Reports) had accumulated byte-identical filter-state, cursor-pagination, and (Audit/Events) mod-events subscription machinery over the cycle; the duplication was deliberate (build first, extract once the shape settled). `AuroraListPage` (`static/admin/scripts/components/ListPage.js`) now holds those routines once — `readFilters`/`applyFilters` (the URL-state round-trip), `renderPagination` (the cursor-stack prev/next), and `subscribeModEvents`. Deliberately composable **helpers**, not a page factory: state stays in each page (its own `mount`/`refresh` control flow + `cursorStack`/`nextCursor`/`lastFilters`), and `renderPagination` mutates the page-owned cursor array in place, so behavior is preserved exactly — only the duplicated routine bodies moved. Each page keeps everything page-specific (endpoint + params mapping, columns/cards, empty/error copy, Reports' `status='open'` default + bulk-bar, Audit's chain indicator + client-side `verifiedOnly` filter, Events' live-prepend). Recon scoping: the kickoff's enumerated recovery pages (RepoRebuild/RepoRepair/SequencerRecovery) are wizards/reports, not list pages, and the named Kryphocron domain list pages don't exist; the load-all pages (Accounts/Invites/Queue) and the hybrid Sessions page are a different shape and stay bespoke. −87 net lines across the four pages; static-pin coverage for the helper API + that each page composes it and no longer inlines the machinery. (#257)
- block-cascade machinery — the security-sensitive heart of the #280 arc, completing it (#281 entry-point + #282 cascade). `createBlock` now removes the blocked subject from the blocker's own list-mode audiences, and the `graph.block` routes #281 deliberately left dark are mounted. Shipped in two commits. **(1) Module move:** the cascade-token machinery (`CascadeSource`/`CascadeToken`/`CascadeContext` + mint/verify) moves from `crate::kryphocron` to a new `crate::cascade`, and `mint_token`/`mint_secondary_token` narrow to `pub(in crate::cascade)` — a compile-time confinement so no module outside the cascade machinery can forge cascade authorization (the forgery invariants — token is not `Clone`/`Copy`/`Serialize` — are asserted by `compile_fail` doctests). **(2) The feature:** two type-distinct errors so the handler can route the failure classes (`KryphocronCascadeWriteRejected` → 403, `SwapCidMismatch` → 409; the `apply_writes` swap-CAS arm is retyped off untyped `Validation`). `bind_pipeline`'s Cascade arm gains the §2.4.1 shape stage — after the token authenticates, an exhaustive `CascadeSource` match (no wildcard) enforces, for `BlockCascade`, originator == repo owner + target == `policy.audience` + operation == `Update` + `swap_cid` present, then emits a Cascade-origin `KryphocronAudienceUpdated` (`cascade_id` correlation, `members_removed: [subject]`); non-`BlockCascade` sources are hard rejects (only `BlockCascade` is wired this arc). The orchestration (`crate::cascade::run_block_cascade`, where minting is confined) walks the blocker's audiences paged, removes the subject from each list-mode audience's live-read value (editing the decoded tree in place so unknown fields survive), pins each `Update` with `swap_cid` = the CID just read (read-then-CAS TOCTOU safety with no actor tx in bind), mints one token per write, and routes three outcomes: a shape reject aborts the pass loudly and leaves the `pending`/`pass_start` unpaired (the recovery signal), while a swap mismatch or transient is a best-effort skip counted as `failed`. Observability is a pair so a mid-pass crash is detectable: `KryphocronBlockChanged` `pending`→`completed` events (Category C) and `block-cascade.log` `pass_start`/`pass_end` JSONL (the durable artifact the #225 `getBlockCascadeImpact` reader parses). `deleteBlock` is forward-only (records a `Deleted` event + `removed: 0`, never restores membership). §16 clarification: the subject DID is carried on the `CascadeContext`, not recovered by reading the block record in the bind arm as rev4 §2.5 D5 specced — `bind_pipeline` is a free function with only the audit `shared_tx` and no actor-store access (ST-2), the same constraint that put the members-diff on the handler side; intent (forensic-only `CascadeSource`, subject available to the bind arm) preserved. No kryphocron substrate change. (#282)
- graph.block entry-point substrate (Arc H §7.2.5, #280 arc / #281): the user-facing block write surface on the same DedicatedEndpoint/CapabilityClass::User path as the four existing kryphocron endpoints — NSID_BLOCK + createBlock/deleteBlock handlers, no new CapabilityKind, no kryphocron substrate change (rev4 D1). build_deny_map gains Source-1 overrides so generic createRecord/deleteRecord on graph.block returns 400 RequiresDedicatedEndpoint (Create→createBlock, Delete→deleteBlock; Update stays NotYetSupported). New AuthenticatedDid newtype (rev4 LB-5/H-7), scoped to the cascade boundary per the §16 clarification — rev4's literal "type-proof Boundary-1 everywhere via for_writer" was unachievable (no AuthResult type; actor_store↔api layering bar; the rewrite-on-rotate system for_writer caller), so authenticated_did_for_repo returns AuthenticatedDid and the four endpoints + block helpers consume it through apply_single_create/apply_single_delete, while for_writer keeps String; Boundary-1 is type-enforced at the dedicated-endpoint boundary incl. createBlock (the only #280 cascade entry). Route-not-registered (rev4 F6/M-5): routes() builds the block Router but does NOT merge it (handlers referenced → no dead code; route absent from the returned router) — a createBlock that persisted a block without cascading the audience removals would be a silent privacy failure, so the public route stays dark until #282 wires the cascade and flips the one-line merge. Unblocks #282. (#281)
- Arc H §7.4.2 sequencer recovery — COMPLETE (epic #278), and with it **Arc H is complete across the cycle** (4 of 4 recovery surfaces: §7.3.2 recovery-mode status #276, §7.4.1 repository rebuild #286/#290/#288, §7.4.3 repository repair #279, §7.4.2 sequencer recovery #278). Recon defined the scope honestly — one read-only deep-validation operation ships (#294 substrate + #295 UI); prune/re-sequence/blob-repair defer with documented carryforward. (#278)
- sequencer-recovery admin UI (Arc H §7.4.2, 278b / #295, 0-Rust): a SuperAdmin surface at #ops/sequencer/recovery (full+reduced) consuming the four #294 XRPCs. Sequencer-state panel (total/invalidated events, head/earliest seq, last-validation summary) + the single validation card: a read-only run (no destructive-confirm — it's a diagnostic) → poll getSequencerRecoveryProgress through the walk (events scanned + cancel) → report (clean-bill line, or sample tables of undecodable blobs and non-monotonic commits); the state panel refreshes on finish. Single-flight 409 recovers via a plain progress read (deployment-singleton). Four endpoint wrappers; seqRecovery.* i18n; route nested under the Sequencer page + Operations sidebar entry. Tests: bare-Node static pins for the endpoint NSIDs/verbs, page consumption, the validate dispatch, route/sidebar wiring, and i18n-key completeness. Completes §7.4.2 (#294 + #295). (#295)
- sequencer-recovery substrate (Arc H §7.4.2, 278a / #294): the escalation surface beyond §5.4.4's routine pause/resume/reset. Recon narrowed scope to one honest operation — a read-only DEEP INTEGRITY VALIDATION (Sequencer::validate_integrity): walk every live row ascending, decode each event blob, and flag undecodable blobs (the firehose silently drops them — no other observability path) + per-DID rev non-monotonicity (concurrent-write ordering bugs); capped anomaly samples + true counts; per-row progress, per-page cancel. Deliberately does NOT flag seq gaps (hard-delete leaves expected gaps). SequencerRecoveryJob (deployment single-flight, ScanJob-shaped) behind the generic sequencerRecoveryOptions / runSequencerRecovery (operation:"validate"; 400 otherwise) / getSequencerRecoveryProgress / cancelSequencerRecovery XRPCs; emits the new SequencerValidated audit (§16 D1, Category C); describeCapabilities superadmin family → 17. DEFERRED with carryforward (recon-grounded): prune-invalidated (NO production path sets invalidated=1 — account deletion hard-deletes — so prune targets an always-empty set; precondition: soft-delete semantics), re-sequencing (precondition: a subscriber-cursor-migration protocol; gaps aren't data loss), and malformed-blob repair (routes to §7.4.1 rebuild). Unblocks #295 (UI). (#294)
- Arc H §7.4.3 repository repair — COMPLETE across the cycle (epic #279): the across-accounts scan-then-repair scrub, built as the cascade-of-primitives the recon anticipated — scan (#291) reuses #289's reconstruction, repair (#292) reuses #290's per-account rebuild, UI (#293) consumes both. Only §7.4.2 (sequencer recovery) remains as Arc H State C. (#279)
- bulk repository-repair admin UI (Arc H §7.4.3, 279c / #293, 0-Rust): a SuperAdmin surface at #ops/repo-repair (full+reduced) consuming the seven #291/#292 XRPCs. Scan controls (run / poll getScanProgress through the walk / cancel) → findings panel (getRepoScanResults: severity counts + filter + a per-account table with selection checkboxes, severity badges, live→reconstructed heads, load-more pagination) → repair panel ("Repair selected (N)" / "Repair all" behind a typed-REPAIR + rationale destructive-confirm → repairRepos → poll getBulkRepairProgress: processed/total + repaired/skipped/failed + current account + cancel, honest that an account mid-rebuild finishes atomically). Single-flight 409s on scan + repair recover via a plain progress read (deployment-singleton — no job-id juggling). Seven endpoint wrappers added to endpoints.js; repair.* i18n keys; route + Operations sidebar entry. Tests: bare-Node static pins for the endpoint NSIDs/verbs, page consumption, the typed-REPAIR gate (all + selected), route/sidebar wiring, and i18n-key completeness. Completes §7.4.3 (#291 + #292 + #293). (#293)
- bulk repo-repair substrate (Arc H §7.4.3, 279b / #292): act on #291's scan findings in bulk. BulkRepairJob (deployment single-flight, ScanJob-shaped) iterates target DIDs and drives a per-account rebuild for each through the rebuild registry — reusing #290 rather than reinventing: start()'s spawn body was extracted into a shared drive(), and a new awaitable RebuildRegistry::run_one registers + drives a rebuild to completion under the same per-DID single-flight (start() unchanged). So every bulk repair runs under the same single-flight + emits the same standalone RepoRebuilt audit as an operator-triggered rebuildRepo; a concurrent rebuild of the same DID is detected (Conflict) and skipped. Per-account failure/conflict is tallied (repaired/skipped/failed), never fatal; cancel is observed before each account (an in-flight swap finishes atomically). Three SuperAdmin XRPCs: repairRepos (POST, rationale-required; `all` = every current finding via ScanFindingsStore::all_dids, or an explicit `dids` subset; 409 single-flight; 400 empty target set), getBulkRepairProgress (GET), cancelBulkRepair (POST); describeCapabilities superadmin family → 13. Emits the BulkRepairInitiated envelope audit (§16 D1, Category C — batchId + targetDids + rationale) at batch start; per-account RepoRebuilt events stay standalone and correlate via the DID list (no per-account emit path touched). Unblocks #293 (UI). (#292)
- bulk repo-repair scan substrate (Arc H §7.4.3, 279a / #291): the across-accounts "scrub" that detects repo-vs-sequencer inconsistencies for later repair. ScanJob (deployment single-flight, RewriteJob-shaped, cancellable per account boundary) walks every account and classifies it via repo_scan::classify reusing #289's reconstruct_and_verify(signing_did_key=None) fast path — one structural reconstruction yields both the head CID (for the comparison) and the unreconstructable signal. Category-based severity (locked): high = reconstruction fails / live repo unrebuildable; medium = reconstructed head ≠ live head; low = heads match but rev differs. Findings persist in a new repo_scan_finding table (migrations 0016 SQLite / 0017 Postgres, backend-identical, natural PK (scan_id, did)) via ScanFindingsStore; a new scan clears the prior set (latest-scan-retained). Four SuperAdmin XRPCs: scanReposForInconsistencies (POST, 409 single-flight), getScanProgress (GET), cancelScan (POST), getRepoScanResults (GET — severity filter, keyset pagination, severity counts); describeCapabilities superadmin family → 10. Emits the new ScanCompleted host-vocabulary audit event (§16 D1, Category C) with the severity breakdown on completion. The classification decision is a pure function over (reconstruction-outcome, live-head), exhaustively unit-tested. Unblocks #292 (bulk repair). (#291)
- repository-rebuild admin UI (Arc H §7.4.1, 277c / #288, 0-Rust): a SuperAdmin recovery surface at #ops/repo-rebuild[/:did] (full+reduced) consuming the four #286/#290 XRPCs. DID input → pre-rebuild check (shallow metadata, or deep=true full reconstruction+verification — a deep failure renders the diagnostic inline and blocks the rebuild) → destructive-confirm (typed REBUILD + rationale, flowed into the RepoRebuilt audit) → phase-aware progress polling (2s walking / 5s verify+swap) through walking/verifying/swapping → terminal summary (completed head-before→after / cancelled-original-untouched / failure diagnostic). Cancel affordance is explicit about per-phase semantics (clean before the swap, atomic-uninterruptible during); single-flight 409 recovers the in-flight job-id from the conflict message and resumes polling. Four XRPC wrappers added to endpoints.js; rebuild.* i18n keys; route + Operations sidebar entry. Tests: bare-Node static pins for endpoints/NSIDs, page consumption, the typed-REBUILD gate, route/sidebar wiring, and i18n-key completeness; drive-by repair of the stale SettingsRoles→ConfigRoles refs in endpoints.test.js (silently failing since the v0.9 rename). Completes §7.4.1 across the cycle (#286 + #290 + #288). (#288)
- repo-rebuild destructive substrate (Arc H §7.4.1, 287b / #290): rebuild::atomic_swap replaces an account's live repo in ONE per-DID SQLite transaction (wipe record + repo_block, insert the reconstructed blocks, set repo_root to the reconstructed head, rebuild the record index from the MST leaves; commits/rolls-back whole). #289's reconstruct_and_verify refactored into shareable accumulate_history + verify_reconstructed; the rebuild runs full-signature verify_repo with the account's plc_keys→did:key (the path importRepo uses). Driven by a background RebuildJob behind a registry — phase + progress, cancel at commit/phase boundaries (the atomic swap is the point of no return), per-DID single-flight (409 while in flight); restart-resume is YAGNI (a restart = failed-rebuild-original-untouched, operator retries). Three SuperAdmin XRPCs: rebuildRepo (POST, rationale-required), getRebuildProgress (GET), cancelRebuild (POST). Emits the new RepoRebuilt host-vocabulary audit event (§16 D1, Category C own tx) on a successful swap. §7.4.1 verification gate locked end-to-end: post-swap root + block set + record index equal a fresh reconstruction, stale state is obliterated, and the rebuild is deterministic across passes. Unblocks #288 (UI). (#290)
- repo-rebuild reconstruction+verify primitive (Arc H §7.4.1, 287b-prep / #289): rebuild::reconstruct_and_verify walks an account's full commit history via Sequencer::commit_events_after (ASC paging), folds each #commit CAR delta into one accumulating BlockMap, and roots at the head through proto-blue verify_repo — structural-only (signing_did_key=None), non-destructive. Consumed by a deep=true mode on tools.aurora.superadmin.preRebuildCheck: reports deepVerified plus reconstructedHeadCid/reconstructedRev; a reconstruction that fails verification is a 200 diagnostic (deepVerified:false + deepError), not a 500. The destructive in-memory→swap (RebuildJob, rebuildRepo/getRebuildProgress/cancelRebuild, RepoRebuilt audit) is #290. (#289)
- repo-rebuild preflight (Arc H §7.4.1, 277a / #286): Sequencer::rebuild_preflight walks an account's full commit history ascending (pages by seq) and aggregates from event metadata — commit count, net live record count (Σ create−delete), rev range, head commit CID; tools.aurora.superadmin.preRebuildCheck (GET, SuperAdmin, non-destructive) surfaces it for scope confirmation. Block/MST reconstruction + the destructive shadow-then-swap apply are #287.
- Arc C Chapter 6 — extension-points docs (authoring-themes.md §12.3 outline) + examples/extension-point-author reference theme (#284)
- extension-point runtime (§11.7 / #285): step-10 manifest validation (declared providedExtensionPoints must be defined as .extension-<name>); /theme/active-extensions.css served (additive chain) + /theme/active-extension-points JSON; AuroraThemeRuntime.themeProvidesExtension(name) (cached, fail-soft) for surface opt-in; theme picker surfaces extension points (§11.7.3). Unblocks Arc C Chapter 6 (#284).
- §16 clarification: recovery-CLI parity premise is false (Arc H recon) — §7.4 surfaces are greenfield substrate-novel, not CLI-wrapping (#283)
- block-cascade machinery (#239/#240) reframed after recon as standalone substrate arc #280 — NO code shipped: recon found the graph.block user-facing endpoint + block capability + cascade-authorization arm are all unbuilt (deferred post-arc-2 work), substrate-novel + security-sensitive, not an Arc D tail. Arc D's drawer block-cascade-impact row keeps its honest available:false empty-state (#232) until #281/#282 land in a future 0.9.x.
- §7.3.2 recovery-mode status surface (State A): read-only active/inactive + documented env+restart enter/exit; no runtime controls (substrate is startup-scoped by design) (#276)
...
- I-subscription-leak — §10.5.2 subscription-leakage audit (#259)
...
- I-loading-audit — §8.2.5 loading-state existing-page audit (#248)
- I-error-audit — §8.2.4 error-handling existing-page audit (#247)
- I-loading-primitives — build AuroraSkeleton + AuroraSpinner + apply per §8.2.5 hierarchy (#250)
- I-error-primitives — build AuroraInlineError + AuroraErrorBoundary + apply per §8.2.4 hierarchy (#249)
- I-source-tier — §8.2.1 source-tier indicator: consolidate primitive + backfill coverage (#246)
- I-modal-discipline — §8.2.3 confirmation-modal audit (labels / typed-confirm / rationale / a11y) (#245)
- I-color-drift — §10.4.4 color-usage drift: tokenize hardcoded colors in component CSS (#244)
- I-timestamp — §10.4.3 time-format consistency: AuroraTimestamp primitive + migrate ad-hoc formatters (#242)
- I-naming — §10.4.5 'Admin UI' naming audit: resolve to canonical 'Aurora-Locus admin' (#241)
...
- D-tier-activity — Public/Private tier record write counts + trends page (read-only) (#231)
- D-audience-aggregate — deployment-wide audience statistics page (read-only) (#230)
- D-laquna — Laquna page (current state + one-button rotation + cadence config) + rotation-history sub-page (#229)
- D-overview — Kryphocron Overview page (substrate identity/codec/rotation status/aggregate counts/activity feed) (#228)
- D-policy-page — Configuration -> Kryphocron policy (audience defaults + laquna cadence range + active-codec display read-only); replace stub (#227)
- D-routes — four Kryphocron routes (overview/laquna/laquna-history/audiences/tier-activity) + role x mode gating; retire the bare stub route (#226)
- D-read-endpoints — the ten `tools.aurora.ops.kryphocron.*` operator read XRPC backing the v0.9 Kryphocron domain pages (§6.4, §6.5), under one new `kryphocron-read-v1` wire capability. Substrate/observability reads (Moderator+): `getSubstrateInfo` (kryphocron version + lexicon-registry hash + codec id + oracle identity + install-validation outcome + embedded aggregate counts), `getTierStats` (per-NSID counts, accounts-with-N-records distribution, 30-day per-NSID time series across the 8 NSIDs), `getOracleActivity` (stub — the substrate build ships no oracle-consultation instrumentation), `getRotationStatus` (active generation mark, last slug rotation, next scheduled rotation, last record-rewrite-completed, in-progress flag, cadence), `getAudienceAggregate` (deployment-wide `policy.audience` totals + 5-mode distribution + list-size histogram), `listAudiences?account=` (per-account audience list — name/mode/member-count only, never `members` contents), `getBlockCascadeImpact?account=` (per-account cascade-removal counts; reads `block-cascade.log`, `available:false` until the block handler writes it). Laquna-control reads (Admin+, in-handler `require_admin`): `getRotationProgress` (live rewrite-job state via new `RewriteJob::progress()`), `cancelRotation` (new `RewriteJob::request_cancel()` + an `AtomicBool` cancel flag observed at each account/batch boundary → `RewriteOutcome::Aborted`, recorded as an `aborted` `rewrite-history.log` entry), `listRotations` (two-track history — operator-triggered from #224's `rewrite-history.log`; the cadence-organic track reads `rotation-history.log`, which the oracle does not yet write, so it is empty for now; equal timestamps sort operator-triggered first per §13.2). New read-only oracle accessors `current_mark()` / `last_rotation_at()` never trigger a rotation as a side effect. Ships `getAudienceAggregate` and the per-account `listAudiences` as distinct endpoints per design §6.8/§7.2.1 — the kickoff's single `listAudiences` conflated the §6.4.3 deployment aggregate with the §6.5 account-drawer list. (#225)
- D-rewrite-on-rotate — `triggerRotation` now rotates the Laquna slug **and** spawns a background job that re-encodes every local account's `tools.kryphocron.feed.postPrivate` records under the new generation: decode each under its write-time generation, re-encode under the post-rotation generation, re-store via an `Update` (at-URIs stable, CIDs change). Updates batch per account (≤100/commit) so a rewrite emits one `#commit` per batch. Single-flight — a concurrent `triggerRotation` while a rewrite is in progress returns HTTP 409 "rotation already in progress" and does not rotate. Host-vocabulary bookkeeping (per §16 D1): `rewrite-history.log` + `last-rewrite-completed.state` under `<data-dir>/aurora-locus/`. The read side (`getRotationProgress` / `getRotationStatus` / `cancelRotation`) lands with its XRPCs in #225. (#224)
- D-read-bind — decode-on-read for private-tier records via `com.atproto.repo.getRecord`. An authorized reader fetching a `tools.kryphocron.feed.postPrivate` record now receives decoded `text`; a non-authorized or anonymous reader receives the encoded form unchanged (transparent — atproto's `getRecord`-is-public contract and the friction model are both preserved). Aurora-Locus owns the authorization decision (resolver mirrors the write-side `check_participate_audience`, fail-closed: self-read always allowed; list-mode audience membership honored; non-list/cross-DID/unverifiable/anonymous deny) and decodes via the kryphocron 0.3.1 at-rest seam (`EncodedRecord::new`/`DecodeContext::new` → `ContentCodec::decode`); cross-codec skew returns HTTP 410. Scoped to `getRecord`; `sync.getRepo`/CAR-export/firehose propagate the encoded form (self-export + migration are separate future tickets). Also bumps kryphocron 0.3.0 → 0.3.1 (host-constructible decode inputs). (#237)
- D-encode-seam — private-tier records (`tools.kryphocron.feed.postPrivate`) authored via `createPostPrivate`/`participatePrivate` now have their `text` content encoded at rest through the installed kryphocron codec (Laquna by default) on write, landing as `encodedContent`/`encodedContentCodec`/`encodedContentGeneration` rather than plaintext — the encoding-at-default floor is now structurally enforced on the host write path, not just validated at boot. The at-rest hooks (codec + #223 rotation oracle) are held in `AppContext` and consumed at every write. Decode-on-read (authorized `getRecord` + self-CAR-export) was split to #237 (D-read-bind) after source-trace found the decode path needs the full read-side capability-bind subsystem. (#236)
