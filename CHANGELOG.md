...
- The Dashboard shows a real account-growth sparkline (Admin+) over the last 30 days, off account creation dates, with a header toggle between new-accounts-per-day and the cumulative deployment total. This replaces a synthetic placeholder series and removes the last external CDN dependency (Chart.js) — the admin UI now loads no third-party scripts
